...
- The appearance of the main interface has been improved.
    ...
    - Now the dlc logo at the moment has been added to the background of module cards as a watermark.
- An issue of unexpected success notification after copying a credential while not logged in has been fixed.
...
### Auto Display ID [UI_Optimization]

- Now the moduel supports displaying `Status` ID as well as `Weather` ID in Minimap.
- The update logic of `Target` ID has been improved.

### Field Operation Entry Command [Assistance]

- Now the module supports joining `Auxesia` using the parameter `auxesia`.
- Now the module supports auto dismounting.